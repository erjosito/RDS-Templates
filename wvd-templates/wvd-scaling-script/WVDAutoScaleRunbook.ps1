...
if ($LogAnalyticsWorkspaceId -and $LogAnalyticsPrimaryKey)
{
	...
	Write-Output "Starting WVD tenant hosts scale optimization: Current Date Time is: $CurrentDateTime, current week day is $CurrentDayOfWeek"
	$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "Starting WVD tenant hosts scale optimization: Current Date Time is: $CurrentDateTime, current week day is $CurrentDayOfWeek" }
	Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
				...
				if ($SessionHost.Status -ne "NoHeartbeat" -or $SessionHost.Status -ne "Unavailable") {
					if ($NumberOfRunningHost -gt $MinimumNumberOfRDSH) {
						...
						if ($SessionHost.Sessions -eq 0) {
							...
							try {
								$KeepDrainMode = Set-RdsSessionHost -TenantName $TenantName -HostPoolName $HostpoolName -Name $SessionHostName -AllowNewSession $false -ErrorAction Stop
							}
							catch {
